...
class FWS_Admin_Page {
    ...
    public function enqueue_admin_assets($hook) {
        ...
        wp_localize_script('fws-admin', 'fws_admin_params', array(
    ...
    public function ajax_search_products() {
        ...
        foreach ((array) $rows as $row) {
            $results[] = array('id' => (int) $row->ID, 'text' => $row->post_title);
        }
    ...
    public function ajax_delete_manual_rule() {
        ...
        FWS_Database_Miner::purge_cache();
        wp_send_json_success(array('message' => 'قانون دستی حذف شد.', 'count' => count($kept)));
    }
    ...
    public function ajax_add_blacklist_product() {
        ...
        FWS_Database_Miner::purge_cache();
        wp_send_json_success(array('message' => sprintf('«%s» به لیست سیاه اضافه شد.', get_the_title($pid)), 'count' => count($list)));
    }
    ...
    public function ajax_remove_blacklist_product() {
        ...
        FWS_Settings::persist_key('product_blacklist', $list);
        FWS_Database_Miner::purge_cache();
        wp_send_json_success(array('message' => 'محصول از لیست سیاه حذف شد.', 'count' => count($list)));
    }
    ...
    private function render_settings_card($settings) {
        ...
        $style_keys = array(
            'style_master_enable', 'style_preset',
            'enable_widget_product', 'enable_widget_cart', 'enable_widget_thankyou',
            'enable_widget_shipping', 'enable_widget_account', 'enable_search_banner',
            'accent_color', 'accent_text_color', 'badge_bg_color', 'badge_text_color',
            'box_bg_color', 'box_border_color',
            'inherit_theme_font', 'force_rtl', 'border_radius', 'base_font_size',
            'hide_confidence_tags', 'show_emojis', 'custom_css',
        );
